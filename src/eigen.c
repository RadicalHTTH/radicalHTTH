 ...
 *  was written by Tianlin Wang at University of Illinois.
 *  Add annotations by Hayate Takeuchi (Final Edit: December 14 2025)
 ***********************************************************/


#include "toolsfromPAML.h"

/* Define precision settings and limits for numerical calculations */
#define BASE        2               /* base of floating point arithmetic */
#define DIGITS     40               /* no. of digits to the base BASE in the fraction */
#define MAXITER    100              /* max2. no. of iterations to converge */
#define pos(i,j,n) ((i)*(n)+(j))    /*For one-dimensional access of matrices */
...
int realeig(int job, double mat[], int n,int low, int hi,
            double valr[], double vali[], double vr[], double vi[]);
void elemhess(int job, double mat[], int n, int low, int hi,
              double vr[], double vi[], int work[]);

/* Main function for eigenvalue and eigenvector calculation */
int eigen(int job, double A[], int n, double rr[], double ri[],
    ...
    */
    ...
    double t, tiny = sqrt(pow((double)BASE, (double)(1 - DIGITS)));

    /* Convert A into a balanced matrix and identify the shrinkage area (scaling) */
    balance(A, n, &low, &hi, work);

    /* Reduce A to Hessenberg form */
    elemhess(job, A, n, low, hi, vr, vi, (int*)(work + n));

    /* Calculate eigen alues ​​and eigen vectors */
    if (realeig(job, A, n, low, hi, rr, ri, vr, vi) == -1) return -1;

    /* Reverse scaling: Return to original scale */
    if (job) unbalance(n, vr, vi, low, hi, work);

    /* Sort, added by Z. Yang */
    /* Sort by absolute eigen values ​​in descending order and sort eigen vectors */
    for (i=0; i<n; i++) {
...
/* Function to create the complex number a + bi */
...
/* Calculate the complex conjugate: conj(a + bi) = a - bi */
//complex conj(complex a) {
//    a.im = -a.im;
//    return a;
//}

/* Returns the size of a complex number */
...
/* Scalar product of real number a and complex number x */
...
/* Function that copies a complex vector x[] to y[] */
...
/* Function to compute complex matrix multiplication */
...
/* Function to calculate the inverse of a complex matrix
 * x[n*m]  ... m>=n
 */
int cmatinv(complex x[], int n, int m, double space[])
{
    ...
    FOR(i, n) {
        ...
        /* Swap rows (pivot row to ith row) */
        if (irow[i] != i) {
            FOR(j, m) {
                t = x[i*m+j];
        ...
        /* Set column i of other rows to 0 */
        FOR(j, n) {
            if (j == i) continue;
            t1 = cby(t, x[j*m+i]);
            FOR(k, m)
...
/* Balancing matrices prior to computing eigenvalues ​​and eigenvectors */
void balance(double mat[], int n, int *low, int *hi, double scale[])
{
    double c, f, g, r, s;
    int i, j, k, l, done;
    ...
    /****************************************************************
     * Step 1: Move down the rows where the eigenvalues ​​are separated
     ****************************************************************/
    for (k = n - 1; k >= 0; k--) {
        for (j = k; j >= 0; j--) {
            for (i = 0; i <= k; i++) {
    ...
    /****************************************************************
     * Step 2: Move the columns that contain the eigenvalues ​​to the left
     ****************************************************************/
...
void unbalance(int n, double vr[], double vi[], int low, int hi, double scale[])
{
    ...
    /****************************************************************
     * Step 1: Return the scaled rows to original scale
     ****************************************************************/
    ...
    /****************************************************************
     * Step 2: Revert rows that were swapped during balancing
     ****************************************************************/
    ...
    /****************************************************************
     * Step 3: Also restore rows larger than hi
     ****************************************************************/
