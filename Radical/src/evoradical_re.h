...
#define MAXLINE	      256
#define NAMELENGTH    10    //length of sequence names
#define NS            40    //maximum # of sequences
...
#define NNODE         (NS*2-1)
#define NEDGE         (NS*2-2)
#define NP	          100	//max # of parameters that needed to be optimized
#define MAXSTRING     100
#define PENALTYUPPER  1
#define PENALTYLOWER  1
#define GTOL          1.0e-6
#define MAXBRANCHES   10  //maximum number of foreground branch
...
extern int is_foreground_branch[MAXBRANCHES];  // foreground branch のフラグ（1/0）
extern int *site_class;  // [No_codon] 各サイトのカテゴリ（0〜3）
extern double gamma0, gamma1;  // gamma 値（背景・前景）
extern double omega0, omega1;  // omega 値（カテゴリまたは前景/背景）
...
double return_fixGammaP(int node_i);  // model==3,4 用のブランチモデル尤度計算
